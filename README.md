...
Current foundation includes the runtime home boundary, initial local directories, versioned public config, strict consultant response validation and `/api/health` plus `/api/bootstrap`. The durable state layer now includes embedded sequential SQLite migrations plus project, job, append only event and approval persistence with explicit identity, policy, runtime, request snapshot, payload integrity, scoped idempotency, expiry and restart recovery contracts.
